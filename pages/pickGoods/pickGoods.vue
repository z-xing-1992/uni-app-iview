<template>
	<view class="pickGoods-container">
		<view class="main">
			...
			<view class="select">
				<text class="actove"  :class="{'active': isActive.indexOf(index)!=-1}"  @click="pickCheck(index,item)" v-for="(item,index) in taskReason" :key="item">{{item.name}}</text>
			</view>
			<view class="title">
				<text>上传照片:</text>
			</view>
	...
	export default {
		...
		methods: {
			pickCheck(index,item){
				...
				let arr=[]
				for(let i=0;i<that.taskReason.length;i++){
					if(that.taskReason[i].check==true){
						arr.push(that.taskReason[i].name)
					}
				}
				that.operType=arr.toString()
			},
			getErrorReason(){ //获取异常原因
				let that=this
				that.$request({
					url: '/data/taskReason',
					method: 'GET',
					...
					if(res.resultCode==0){
						for(let i=0;i<res.data.length;i++){
							that.taskReason[i]={name:res.data[i],check:false}
						}
					}
				}).catch(erro => {
			...
			sureResaon(){//确认提交
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							let abnormal = uni.getStorageSync('abnormal')
							uni.getLocation({
								type:'gcj02',//
								success:function(result){
									that.$request({  
										url: '/task/startTask?taskId='+abnormal.id,
										method: 'POST',
										data:{
											phone:abnormal.phone,
											lat:result.latitude,
...
<style>
	...
	.pickGoods-container{width: 100%;height: 100%;background: #F6F5F5;}
